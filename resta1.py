    ...
    for y in range(len(game_map)):
        for x in range(len(game_map[0])):
            ...
            if game_map[y][x] != 'x' and square_x <= 6 and square_y <= 6 and square_x == x and square_y == y:
                pg.draw.rect(window, purple_light, ((square_x * size) + off_set - (size / 2), (square_y * size) + off_set - (size / 2), size, size))
                if mouse[2][0]:
                    last_selected[0] = int(selected[0])
                    last_selected[1] = int(selected[1])
                    selected[0] = int(square_x)
                    selected[1] = int(square_y)
                    return selected, last_click_status
...
def select():
    ...
    pg.draw.rect(window, purple_dark, 
                 ((selected[0] * size ) + off_set - (size / 2),
                  (selected[1] * size) + off_set - (size / 2),
                  size,
                  size))
...
def restart_b(mouse):
    ...
    pg.draw.rect(window, green, (window.get_width() * 0.64, window.get_height() * 0.20, 250, 75), 7)
    texto = fonte.render('Restart', 1, black)
    window.blit(texto, (window.get_width() * 0.64, window.get_height() * 0.22))
